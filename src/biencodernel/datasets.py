...
class MentionDataset(torch.utils.data.Dataset):
    def __init__(self, path_to_json: str, max_length: int, kb_dataset: KBDataset, ms_token='[ms]', me_token='[me]',
                 bert_model: str = 'bert-base-german-cased'):
        """
        ...
        for doc in tqdm(dataset.documents, desc='Processing dataset {}'.format(dataset.name)):
            for ann in doc.annotations:
                ...
                tokens = TokenTools.center_mention(tokens=tokens, max_length=self.max_length,
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, str]:
        """
        :param idx:
        :return: mention tokens, concept tokens, concept_id
        """
        return self.tokenized_mentions[idx], self.tokenized_kb_concepts[idx], self.concept_ids[idx]


class MentionDatasetNoNER(torch.utils.data.Dataset):
    def __init__(self, path_to_json: str, max_length: int, kb_dataset: KBDataset,
                 bert_model: str = 'bert-base-german-cased'):
        """
        Initializes a TeXoo MentionDataset for NEL training that creates data loaders from TeXoo JSON files
        :param data_path: path to folder structure
        ...
        self.max_length: int = max_length
        self.kb_dataset: KBDataset = kb_dataset
        self.ms_token: str = '[ms]'
        self.me_token: str = '[me]'
        self.bert_model = bert_model
        self.tokenizer: BertTokenizer = TokenTools.get_tokenizer(ms_token=self.ms_token, me_token=self.me_token,
                                                                 bert_model=self.bert_model)
        self.tokenized_mentions: List[torch.Tensor] = []
        ...
        for doc in tqdm(dataset.documents, desc='Processing dataset {}'.format(dataset.name)):
            for ann in doc.annotations:
                ...
                tokens = self.tokenizer.tokenize(mention_with_context)
                tokens = TokenTools.center_mention(tokens=tokens, max_length=self.max_length + 2,
                                                   ms_token=self.ms_token,
                                                   me_token=self.me_token)
                tokens.remove(self.ms_token)
                try:
                    tokens.remove(self.me_token)
                except ValueError:
                    # if me_token got cropped away by the center_mention max_length constraint,
                    # remove last content token instead.
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, str]:
        """
        :param idx:
        :return: mention tokens, concept tokens, concept id
        """
        return self.tokenized_mentions[idx], self.tokenized_kb_concepts[idx], self.concept_ids[idx]
